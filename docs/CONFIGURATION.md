...
Advice is shown as an Advisor message and persisted in the primary transcript. Notes delivered during an active tool batch fold into the next agent step without cancelling the tool. `concern` and `blocker` notes may steer a response that is currently streaming. Any advice delivered after the primary settles starts a corrective agent turn, so a run never ends on an unaddressed Advisor message.
