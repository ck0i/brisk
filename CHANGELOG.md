...
- `brisk doctor` is reserved by the CLI but is not implemented in 0.1.0.
- `compaction.enabled`, `ui.theme`, and `ui.showThinking` are accepted configuration fields but do not alter the 0.1.0 interactive runtime.
...
- Cross-compilation is not cross-platform runtime verification. Every standalone artifact must be run on its target OS; the packaged Linux native target is glibc rather than musl.
